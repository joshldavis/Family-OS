...
const Schoolwork: React.FC = () => {
  ...
  const toggleAssignmentStatus = (id: string) => {
    ...
    if (assignment.status === Status.DONE) {
      dispatch({
        ...
        payload: { ...assignment, status: Status.NOT_STARTED, updatedAt: now, completedAt: undefined },
      });
    } else {
      dispatch({
        type: 'COMPLETE_ASSIGNMENT',
        payload: { id, completedById: state.currentUser?.id ?? '' },
      });
    }
  };
  ...
  const handleSaveAssignment = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingAssignment) {
      dispatch({
        type: 'UPDATE_ASSIGNMENT',
        payload: { ...editingAssignment, ...assignmentData, updatedAt: now },
      });
    } else {
      dispatch({
        type: 'ADD_ASSIGNMENT',
  ...
  const handleSaveStudent = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch({ type: 'HYDRATE', payload: { students: [...state.students, newStudent] } });
